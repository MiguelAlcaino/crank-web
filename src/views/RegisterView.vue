...
const submitForm = async () => {
  ...
  if (isValid) {
    ...
    if (response === "SuccessRegistration") {
      ...
      console.log("Error registering user", "The password must contain a letter and a number");
    } else if (response === "MinimumPasswordLengthException") {
      console.log("Error registering user", "The password must contain at least 8 characters");
    } else if (response === "RegisterUserAlreadyRegisteredException") {
      console.log("ERROR", "Email address already registered");
    } else {
      console.log("Error registering user", "Ups! Sorry, we didn't see that coming!. Please try again or communicate with the team to resolve this issue.")
    }
  } else {
